...
// The default parameters assume an empty message
...
function renderEmail(id, mailbox) {
	...
	const singleEmailView = document.querySelector(
		'#single-email-view .mail-container'
	);
  ...
  `;
    ...
    `;
		...
		.then(() => {
			const email = document.querySelector('.single-email');
			const archived = mailbox === 'inbox' ? true : false;

			const emailSubject = email.querySelector('.email-subject')
				.textContent;
			const emailSender = email.querySelector('.email-sender p')
				.textContent;
			const emailBody = email.querySelector('.email-body p').textContent;

			// Add reply and Archuve/Unarchive buttons on Inboxes other than the 'Sent' Inbox
			if (mailbox !== 'sent') {
				...
				email
					...
					.addEventListener('click', () =>
						compose_email(emailSubject, emailSender, emailBody)
					);
				email
				...
				archived
					? (email.querySelector('.archive').textContent = 'Archive')
					: (email.querySelector('.archive').textContent =
							'Unarchive');
			}
		});
